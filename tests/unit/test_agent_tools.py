...
async def test_execute_retrieve_success():
    ...
    response = await execute_retrieve(mock_pipeline, "auth system", top_k=3)
    
    mock_pipeline.query.assert_called_once_with(query="auth system", top_k=3)
...
async def test_execute_retrieve_no_results():
    ...
    response = await execute_retrieve(mock_pipeline, "nonexistent code")
...
async def test_execute_retrieve_exception():
    ...
    response = await execute_retrieve(mock_pipeline, "some query")
